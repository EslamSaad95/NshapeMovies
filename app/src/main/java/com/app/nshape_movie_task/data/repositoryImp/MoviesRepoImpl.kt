...
class MoviesRepoImpl @Inject constructor (private val apiService: ApiService):MoviesRepository {

  override suspend fun getMovies(): ApiResult<List<MoviesEntity>, ApiFailure> {
    try {
      val response = apiService.getMovies()
      if (response.isSuccessful)
        return ApiResult(value = response.body()?.results?.toMoviesEntity())
      else {
